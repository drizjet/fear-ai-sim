...
namespace FearAI.Client
{
    ...
    /// Because it has to. This file ships in a project that targets
    /// **netstandard2.0**, where none of the convenient modern members exist:
    ...
    /// compile gate caught exactly that on the first run, which is the whole point
    /// of compiling the adapter rather than reading it. So this builds the SPKI
    /// DER and the PEM text by hand from `RSAParameters`, uses instance
    /// `RandomNumberGenerator`, and settles hex with a loop. The Unity copy is the
    /// same file for the same reason, so there is one implementation of the
    /// algorithm rather than three subtly different ones.
    ///
    /// KEY STORAGE

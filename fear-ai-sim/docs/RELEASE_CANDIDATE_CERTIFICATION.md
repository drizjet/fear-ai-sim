...
| .NET adapters compile **and the Unity fixtures run** | `node tools/verification/verify_dotnet_adapters_compile.mjs` | PASS (**27 checks** on the 2026-09-21 host run: the Unity control-plane files compile outside the editor against the **shared** UnityEngine shim — the same shim the behaviour probe runs — the standalone C# client is rebuilt with `-t:Rebuild`, and **the Unity EditMode fixtures are compiled and run against the shims**, through `tools/verification/unity/NUnitTestRunner.cs`: **41 cases over 5 fixtures, 0 failed, 0 ignored**, after the runner passed its own outcome self-test, with the cases it ran and the fixtures it found both checked against counts derived from the test sources, so a fixture body that can never pass is caught without an Editor. **This is not an Editor result**: a shim that differs from Unity's API compiles and passes here and fails in Unity. Reports `SKIPPED`, never `PASS`, when `dotnet` is absent) |
| Unity adapter behaviour | `node tools/verification/verify_unity_adapter_behavior.mjs` | PASS on the 2026-09-21 host run (**37 probe assertions across four OS processes**; the unmodified adapter is executed against a live `FearServer` through the shared UnityEngine shim: one-request batched registration, refusal accounting for a tokenless rival on both claim and teardown, the owner's own teardown/trauma unrefused, the credential written to a real file, a **new process loading that credential being GRANTED its crowd** with zero refusals, proof-by-key against a `required`-policy server where a **leaked token alone is refused with nothing mutated**, and the credential **and signing key** written unreadable at rest and read back by the Node reference. Phase 1 also asserts the shim's **lifecycle order** on an instrumented component before asserting the adapter's own — the wake runs the adapter's `Awake`, a frame runs its `async void Start`, and teardown runs its `OnDestroy` — which is 25 of the 37) |
...
| Unity EditMode Editor gate (new) | `node tools/verification/verify_unity_editor_tests.mjs` | **SKIPPED** on this machine (no Unity Editor installed), exit 0, stating that no Editor executed the 41 fixtures and naming what only an Editor can answer — it used to say the tests were *neither compiled nor run*, which stopped being true once they started running against the shims. The fixtures themselves are compiled **and run** by the `verify_dotnet_adapters_compile.mjs` row above. With an Editor present the runner assembles a throwaway project taking the package as a local `file:` dependency, lists it in `testables`, runs `-batchmode -runTests -testPlatform EditMode`, and fails on a red test, a missing results file, or **zero tests executed**. `FEAR_AI_UNITY_REQUIRED=1` turns the skip into a failure. **This row does not promote the Unity adapter** — no Editor has executed these tests |

...
| Verification | `tools/verification/*.mjs` standalone harnesses (25 `verify_*.mjs` probes — 19 runtime/protocol/behaviour, 3 Godot contract/parity/live-wiring, 1 .NET compile-**and-run** check, 1 Unity behaviour harness, 1 Unity Editor gate — plus 2 `measure_*.mjs` measurement harnesses; the five numbers sum to the total, which the integrity gate requires rather than assumes) and 1 external-resource runner (`npm run godot:evidence`) | current — the recorded 2026-09-21 probe run is **25 probes: 24 passed, 1 declared skip, 0 failed**, and the 4/4 in-engine suites were green in the 2026-09-20 sweep. `verify_unity_editor_tests.mjs` reports `SKIPPED` rather than `PASS` (no Editor installed) and the .NET, Godot and Unity multi-process probes report `SKIPPED`, never `PASS`, when their external tools are absent. The count is a count of files, not a claim about coverage |

...
| `packages/core` — information propagation (**opt-in**) | `InformationPropagationEngine`, constructed only behind `enableInformationPropagation` (**default off**) | `VERIFIED_CURRENT (OPT_IN_SERVICE)` — the first module to join this surface from §2 by the policy in §3. **Off by default**: `sim.informationPropagation` is `null`, and a default run is byte-identical — advisory outputs, persistence snapshot and RNG stream — to a run with the flag explicitly `false`, and also to a run with it `true`. When enabled it advances once per tick, tracks the roster (registration joins the listening network; retirement drops that agent's edges, beliefs and contribution to reported reach, and a retired source stops being heard), and exposes `injectRumor` / `correctRumor` / `propagatedInformation`. It refuses rather than silently no-oping when the service is off, when an origin is not a registered agent, or on an unknown topic or rumor id. `auditImmutability()` reports zero host physics and transform mutations, and its own RNG is seeded from the simulation seed so it consumes nothing from `this.rng`. **Advisory only and bounded**: it contributes nothing to fear, band or intent; its state is deliberately **not** in the persistence snapshot, so a host that wants continuity across a restart persists it itself; and its research claims remain `EXPERIMENTAL` (`verify_information_propagation_optin.mjs`, 41 assertions) |
...
| Examples (`examples/`) | The CLI, Python and reference-game walkthroughs, each self-asserting and gated; the Godot, Unity and Unreal examples are **not run** here | `RUN_OR_DECLARED` — `examples/cli/neutral-horror-demo.mjs` is executed and mutation-tested by `verify_contagion_transmission.mjs` (18 claims), the Python demo runs against a server the probe starts (15 claims), and the reference-game runner derives its status from real checks. `verify_examples_inventory.mjs` derives the roster from disk, requires the classification to cover it exactly, and **refuses a success verdict from any file nothing runs**. The six examples needing a Godot binary, a Unity Editor or a UE5 host are declared unrun with reasons rather than counted as coverage |
...
| Verification | `tools/verification/*.mjs` standalone harnesses (28 `verify_*.mjs` probes — 21 runtime/protocol/behaviour, 3 Godot contract/parity/live-wiring, 1 .NET compile-**and-run** check, 1 Unity behaviour harness, 1 Unity Editor gate, 1 examples-inventory gate — plus 2 `measure_*.mjs` measurement harnesses; the numbers sum to the total, which the integrity gate requires rather than assumes) and 1 external-resource runner (`npm run godot:evidence`) | current — the recorded 2026-09-21 probe run is **28 probes: 27 passed, 1 declared skip, 0 failed**, and the 4/4 in-engine suites were green in the 2026-09-20 sweep. `verify_unity_editor_tests.mjs` reports `SKIPPED` rather than `PASS` (no Editor installed) and the .NET, Godot and Unity multi-process probes report `SKIPPED`, never `PASS`, when their external tools are absent. The count is a count of files, not a claim about coverage |
...
**One module has left this list.** `InformationPropagationEngine` is now the
opt-in service in §1, having met all four conditions below in a single change
(§3), and it is listed here rather than silently deleted so the boundary's
history is readable: what left, and why.
...
**Exercised once, on `InformationPropagationEngine`.** Condition 4 is the one
easiest to satisfy dishonestly: deleting a name from the tripwire's negative list
is exactly what an accidental promotion looks like, and a probe that simply stops
checking cannot tell the two apart. So the tripwire **inverted** instead of
shrinking — the module is now required to be constructed, required to default to
`false`, required to be constructed *only* inside its opt-in guard, and required
to be absent from a default `RuntimeSimulation` at runtime. An ungated `new` would
fail the probe. Conditions 1–3 are carried by
`tools/verification/verify_information_propagation_optin.mjs`, whose central
assertion is the negative one: with the service enabled, the simulation's outputs,
its persistence snapshot and its RNG stream must all be **unchanged**, because a
service that quietly perturbed the affect path would be a change to the default
path wearing an opt-in flag's clothes.
...
- It does not close the Unity Editor gap. The Unity control plane is now
  **executed** against a live server outside the Editor, which is strictly
  ...
  WHEN Unity calls the lifecycle, frame scheduling and the shim's fidelity to
  Unity's real API remain unverified, and `FearAgent.cs`/`FearAgentHUD.cs` are
  covered by neither probe. The lifecycle **bodies** are no longer in that list —
  the shim drives `Awake`, `OnEnable`, `Start`, `Update`, `OnDisable` and
  `OnDestroy` in Unity's order, so a body that can never work is caught without an
  Editor — but the timing is the shim's model, not Unity's, so the qualifier
  stands. An EditMode test project now exists in the package and its

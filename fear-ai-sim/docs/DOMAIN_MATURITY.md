...
| law | `LIVE_PATH_INTEGRATED` | `law.js` owns `town.laws` (array of `{id,type,prohibits,scope,penalty}`) with `ensureTownLaws`, `isActionIllegal`, `checkLawCompliance`; `closed-world.js` initializes each town with a `banditry` law on `town-roads`, migrates older saves, checks every `BANDIT_ATTACK` (both direct `resolveBanditAttack` and encounter `BANDIT_ATTACK` consequences) and emits `LAW_VIOLATED` parented to the attack. `tests/law-violation.test.js` (5 tests) proves incident-road violation, empty-law neutrality, scope mismatch honesty, encounter-path enforcement, and save/load persistence. | Laws are per-town but do not yet affect justice/institutional enforcement (no penalty collection, no legitimacy drift). | Slice: wire `LAW_VIOLATED` into `JusticeSystem.resolve` and patrol/institutional response. |

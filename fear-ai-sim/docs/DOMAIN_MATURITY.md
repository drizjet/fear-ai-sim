...
| reputation | `CODE_VERIFIED` | `computeReputation(targetId, observers)` aggregates the per-actor `memoryByActor` across a network of observers (mean of `getMemoryOfLoss(observer, targetId)`). 6 tests prove the §30 contract: no observers → 0, single observer → observer's memory, multiple observers → mean, independent targets, bounded [0, 1] (EVID-2026-08-28-REPUTATION-AGGREGATE). | The aggregation is a mean of raw `memoryByActor` values; no time decay, no observer-trust weighting, no outlier handling. Only the "violence" dimension is implemented (the directive's §30 lists reliability, honesty, trade fairness, etc.). Not yet wired into the closed-world reducer. | Slice: wire `computeReputation` into the closed-world's invasion gate so a faction's invasion target selection considers the target's reputation. |
...
| justice | `LIVE_PATH_INTEGRATED` | `JusticeSystem.resolve` is called per-tick per town. | Justice outcomes are events but do not change faction state. | Slice: have `JusticeSystem.resolve` also adjust `faction.legitimacy`. |
| law | `SPECIFIED` only | No law model exists. | No law system. | Slice: add a `town.laws` map and a `isIllegal(action, actor, town)` predicate. |
...
| performance | `UNIT_VERIFIED` | 84 suites / 867 tests pass; build is clean; Rust 4/4. | No perf benchmark. | Slice: add `tests/closed-world-perf.test.js` that measures tick time over 500 ticks. |

...
superseded_by: docs/CURRENT_TRUTH_LEDGER.md
---

> **Scope:** closed-world / living-systems domains in this JS sim. Not the plug-in SDK maturity.
> Product map: `SYSTEM_MAP.md`. **Historical record** as of 2026-09-20 — see the note at the top of
the body for what replaced it as an authority.
...
**Historical record — superseded 2026-09-20.** The map was hand-promoted while
`docs/evidence/EVIDENCE_LEDGER.jsonl` was maintained; the ledger's last row is dated
2026-09-06 and its gate is **retired**, because thousands of rows no longer re-prove and
forcing green would have meant bulk-invalidating them (`evidence/lint.mjs` refuses with
exit 9 — `npm run lint:evidence`). What replaced it as authority is
`docs/CURRENT_TRUTH_LEDGER.md` (current capability status), with behavior evidence in the
standalone probes (`npm run verify:probes`) and `evidence/*.md`. To see how much of the
closed record still re-proves:

```bash
npm run evidence:report
```

> **Read the status column as history.** No label in the table below is re-proved against
> current sources. The section immediately below is **generated** from the closed ledger,
> so the labels it reports cannot drift out of agreement with the record they came from —
> and it renders any disagreement with the hand-written column instead of hiding it.

<!-- GENERATED:LEDGER-STATUS:BEGIN -->
...
**10,134 rows across 47 domains; last row recorded 2026-09-06T23:48:41.789Z.**
...
**36 domain(s) agree with the hand-written table; 0 disagree (plus 1 declared scope verdict).**
...
4 hand-written row(s) are invisible to the linter's expression because the domain cell is not a single token (`individual cognition`, `trade routes`, `roaming groups`, `chronicle/history`); those were never compared against the ledger in either direction, and no column here covers them.
...
2 domain(s) in the record are receipt-test artifacts, not product domains (`__receipt_test__`, `__supersession__`); they are shown because they are in the ledger.

No label in this table is a statement about today.

| Domain | Ledger claim (as recorded) | Rows | Doc table says | Agrees? |
|---|---|---|---|---|
| `__receipt_test__` | `NO_ROWS` | 0 (+144 retired) | — | — |
| `__supersession__` | `NO_ROWS` | 0 (+4 retired) | — | — |
| `analytics` | `LONG_HORIZON_VERIFIED` | 165 (+1 retired) | `LONG_HORIZON_VERIFIED` | yes |
| `bandits` | `LIVE_PATH_INTEGRATED` | 257 | — | — |
| `belief` | `CROSS_DOMAIN_INTEGRATED` | 332 (+1 retired) | `CROSS_DOMAIN_INTEGRATED` | yes |
| `combat` | `UNIT_VERIFIED` | 124 (+1 retired) | `UNIT_VERIFIED` | yes |
| `convoy` | `SPECIFIED` | 38 | — | — |
| `crime` | `LIVE_PATH_INTEGRATED` | 206 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `demography` | `CROSS_DOMAIN_INTEGRATED` | 336 (+1 retired) | `CROSS_DOMAIN_INTEGRATED` | yes |
| `diplomacy` | `CROSS_DOMAIN_INTEGRATED` | 334 (+1 retired) | `CROSS_DOMAIN_INTEGRATED` | yes |
| `ecology` | `CONSEQUENCE_VERIFIED` | 252 (+5 retired) | `CONSEQUENCE_VERIFIED` | yes |
| `economy` | `LIVE_PATH_INTEGRATED` | 202 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `encounters` | `LIVE_PATH_INTEGRATED` | 206 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `factions` | `LIVE_PATH_INTEGRATED` | 217 | `LIVE_PATH_INTEGRATED` | yes |
| `fear` | `CROSS_DOMAIN_INTEGRATED` | 315 (+1 retired) | `CROSS_DOMAIN_INTEGRATED` | yes |
| `infrastructure` | `LIVE_PATH_INTEGRATED` | 206 | `LIVE_PATH_INTEGRATED` | yes |
| `institutions` | `UNIT_VERIFIED` | 165 (+1 retired) | `UNIT_VERIFIED` | yes |
| `justice` | `CONSEQUENCE_VERIFIED` | 266 (+1 retired) | `CONSEQUENCE_VERIFIED` | yes |
| `law` | `CROSS_DOMAIN_INTEGRATED` | 327 | `CROSS_DOMAIN_INTEGRATED` | yes |
| `leadership` | `UNIT_VERIFIED` | 120 (+1 retired) | `UNIT_VERIFIED` | yes |
| `logistics` | `LIVE_PATH_INTEGRATED` | 206 | `LIVE_PATH_INTEGRATED` | yes |
| `long-horizon` | `LONG_HORIZON_VERIFIED` | 126 | — | — |
| `market` | `CODE_EXISTS` | 74 | — | — |
| `markets` | `LIVE_PATH_INTEGRATED` | 202 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `memory` | `CONSEQUENCE_VERIFIED` | 246 (+1 retired) | `CONSEQUENCE_VERIFIED` | yes |
| `merchants` | `LIVE_PATH_INTEGRATED` | 311 | — | — |
| `migration` | `LIVE_PATH_INTEGRATED` | 206 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `observation` | `CONSEQUENCE_VERIFIED` | 250 (+1 retired) | `CONSEQUENCE_VERIFIED` | yes |
| `patrol` | `LIVE_PATH_INTEGRATED` | 266 | — | — |
| `performance` | `LONG_HORIZON_VERIFIED` | 165 (+1 retired) | `LONG_HORIZON_VERIFIED` | yes |
| `production` | `LIVE_PATH_INTEGRATED` | 231 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `quests` | `SPECIFIED` | 40 | `SPECIFIED` | yes |
| `refugees` | `UNIT_VERIFIED` | 145 | `UNIT_VERIFIED` | yes |
| `relationships` | `CROSS_DOMAIN_INTEGRATED` | 378 | `CROSS_DOMAIN_INTEGRATED` | yes |
| `replay` | `LIVE_PATH_INTEGRATED` | 198 | `LIVE_PATH_INTEGRATED` | yes |
| `reputation` | `CROSS_DOMAIN_INTEGRATED` | 325 (+1 retired) | `CROSS_DOMAIN_INTEGRATED` | yes |
| `resources` | `LIVE_PATH_INTEGRATED` | 202 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `rumor` | `LIVE_PATH_INTEGRATED` | 202 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `runtime` | `RUNTIME_VERIFIED` | 168 | — | — |
| `serialization` | `LIVE_PATH_INTEGRATED` | 206 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `settlements` | `LIVE_PATH_INTEGRATED` | 236 (+1 retired) | `LIVE_PATH_INTEGRATED` | yes |
| `statistical-validation` | `DETERMINISM_VERIFIED` | 210 | — | — |
| `territory` | `CONSEQUENCE_VERIFIED` | 298 | `CONSEQUENCE_VERIFIED` | yes |
| `trade` | `CROSS_DOMAIN_INTEGRATED` | 547 (+1 retired) | `CROSS_DOMAIN_INTEGRATED` | yes |
| `visualization` | `SPECIFIED` | 42 (+1 retired) | `BLOCKED` | NO (scope verdict) |
| `weather` | `LIVE_PATH_INTEGRATED` | 206 | `LIVE_PATH_INTEGRATED` | yes |
| `wildlife` | `LIVE_PATH_INTEGRATED` | 202 | `LIVE_PATH_INTEGRATED` | yes |
<!-- GENERATED:LEDGER-STATUS:END -->
...
| relationships | `CROSS_DOMAIN_INTEGRATED` (with full directional pressure components) | `FactionRelationshipVector` and the `StanceLadder` are wired into the closed-world reducer; the invasion gate consults the pair's stance. The vector supports §15 directional trust + per-perspective `directedTerritorialPressure` / `directedGrievance` / `directedFear` maps (EVID-2026-08-28-TERRITORY-VERTICAL-SLICE). The legacy `trust`, `territorialPressure`, `grievance`, `fear` fields are all derived getters (mean across perspectives) with throwing setters. `recordHarm` / `recordTrade` / `recordTrespass` / `recordIntrusion` accept a `fromFactionId` (or `observerFactionId`) to credit/debit the per-perspective value. **The audit's P1 #1 acceptance test now runs in the live path** (EVID-2026-08-28-PERSPECTIVE-AWARE-CHOOSE-STANCE-LIVE): the closed-world reducer consumes `pair.pressureFrom(fromFactionId)` and `pair.getTrustFrom(fromFactionId)` so A→B and B→A produce independent STANCE_TRANSITION events. `pair.stanceFrom` / `observeFrom` track per-perspective stance memory; the legacy `stance` field is the PEAK observation across perspectives (preserves the invasion-gate query). 6 tests in `tests/directed-relationship-ownership.test.js`, 7 in `tests/live-perspective-aware-choose-stance.test.js`, and 13 in `tests/territory-vertical-slice.test.js` prove the full directional + contextual contract end-to-end. | Factionless actors bypass the vector by design: the invasion gate records TARGET_HAS_NO_FACTION with WHY audit (unaffiliated targets need no inter-faction stance; pinned by tests/stance-invasion-gate.test.js). Debt #1 (territory) and debt #3 (per-perspective components) are both resolved. | Slice: extend the WHY inspector to surface the per-perspective stance trajectories in the runtime debugger. |
...
| justice | `CONSEQUENCE_VERIFIED` | `JusticeSystem.resolve` is called per-tick per town with `{legitimacy,grievance,reportedCrime,investigationQuality,corruption,lawPenalty}`; Slice W adds bounded `lawPenalty` (mean `LAW_VIOLATED` penalty in the 5-tick window, 0 when absent) eroding legitimacy by `penalty*0.15` only when `reportedCrime` is true. `closed-world.js` feeds the per-town window mean, audits `{lawPenalty,lawViolationCount}` on `JUSTICE_RESOLVED`, and parents the event to both attack and law events. The owning faction tracks the outcome via the existing Slice C 0.85/0.15 blend. Restitution (Slice X) moves resources at violation time; justice remains the legitimacy channel. Slice Y gives every violated town its own window input, so starved towns respond. | No law-volume-driven investigation-quality feedback; idle path unchanged (hand-forged LAW without attack cannot erode). | Slice: add law-volume-driven investigation quality. |
| law | `CROSS_DOMAIN_INTEGRATED` | `law.js` owns `town.laws` (array of `{id,type,prohibits,scope,penalty}`) with `ensureTownLaws`, `isActionIllegal`, `checkLawCompliance` (first match, backward compatible) plus `checkAllLawCompliance` (every matching town, insertion order). `closed-world.js` initializes each town with a `banditry` law on `town-roads`, migrates older saves, and checks every `BANDIT_ATTACK` (both direct `resolveBanditAttack` and encounter `BANDIT_ATTACK` consequences) against all towns: each violated town emits its own `LAW_VIOLATED` parented to the attack (Slice Y ended first-match starvation — south now sees road-a). Slice V routes each `LAW_VIOLATED` through `observeLawViolation`: the violated town's `controlledBy` faction records observer-scoped `lawfulness` against the violator faction (bandit `factionId` when known, else raw actor id; self-loops audit-only with no self-observation), and the existing patrol attention consumer reacts without requiring a treaty. Slice W closes the second consumer: the per-town mean `LAW_VIOLATED` penalty erodes `JusticeSystem` legitimacy (`penalty*0.15`, `reportedCrime`-gated) and flows to the owning faction via Slice C — starved towns now respond through their own window. Slice X+Y close the third consumer: penalty-funded restitution transfers resource units from the violator faction to observer factions, apportioned as `townPenalty / executableTownCount` (executable = observer is a real faction distinct from the violator), so the total sentence stays conserved instead of multiplying per town; non-faction violators, missing observers, and self-loops skip honestly with `restitution: null`. `LAW_VIOLATED` audits `{violatorFactionId,observerFactionId,lawfulness,restitution}`. `BANDIT_ATTACK` now carries `factionId` when the attacker bandit has one (omitted for legacy free agents). `tests/law-violation.test.js` (5 tests), `tests/law-lawfulness-enforcement.test.js` (6 tests), `tests/law-justice-penalty.test.js` (6 tests), `tests/law-restitution.test.js` (6 tests), and `tests/law-apportionment.test.js` (6 tests) prove violation, empty-law neutrality, live patrol attention shift, encounter-path faction identity, save/load enforcement identity, free-agent fallback, penalty monotonicity, forged-LAW honesty, resolve backward compatibility, zero-sum transfer, broke/capped boundaries, tick-path audit with persistence, multi-town emission, conserved totals, self-loop audit-only, starved-town justice response, single-scope full sentence, and apportioned persistence. | `tradeFairness`/`honesty` dimensions remain consumer-less (deliberately deferred: no non-overlapping minimal producer found — price-deviation would contradict the opportunity bonus, delivery shortfall is already reliability). | Slice: give `tradeFairness`/`honesty` a live consumer only when a non-overlapping producer is identified; otherwise close `logistics`/`wildlife`/`infrastructure` SPECIFIED gaps. |
| crime | `LIVE_PATH_INTEGRATED` | Towns carry plain-JSON `crime: { investigationQuality }` (default 0.4, the legacy fixed value; older saves migrate on tick). Patrol coverage on an incident road ratchets quality upward (+0.05/tick, cap 0.9); uncovered towns hold steady so patrol-less worlds behave exactly as before. `JusticeSystem.resolve` consumes the town's own quality and `JUSTICE_RESOLVED` audits it. Patrolled towns keep higher legitimacy and lower grievance than bare controls under identical attacks. `tests/crime-investigation.test.js` (6 tests) proves baseline with migration, upward ratchet with cap, legitimacy/grievance split, unrelated-road neutrality, event audit with bare control, and save/load equality with identical follow-up justice. | No unreported-crime modeling; no decay without coverage (ratchet-only by design for baseline stability). | Slice: add unreported-crime estimation (reported vs actual crime gap) or coverage decay. |
...
| markets | `LIVE_PATH_INTEGRATED` | `Market` is used by the closed-world; per-tick `MARKET_TICK` events include per-flow numbers. `getQuote` stays instantaneous while `getElasticQuote` blends a 0.7/0.3 EMA with momentum, and the merchant opportunity bonus prefers it. | No bidding / auction price discovery. | Slice: add a `Market.updatePrice` bid curve or multi-merchant auction. |
...
| trade | `CROSS_DOMAIN_INTEGRATED` | The closed-world merchant delivers cargo through `Market.deliverCargo`. The canonical route decision (`chooseMerchantRouteDecision`) consumes `routing.routeCost` as its base score: identity/belief terms map 1:1 onto a routing perception (10x weights, cost normalized back), so the ranking is identical to the legacy blend by construction while routing owns distance/condition/danger/cargo/familiarity pricing. Market opportunity and trade-reliability adjustments layer on top (no routing equivalent). `tests/routing-merchant-base.test.js` (7 tests) proves base reconstruction, ranking-order identity with routing, danger flip, condition flow, opportunity layering, `selectRoute` agreement, and toll steering (a routing-only term). | The real `trade.js` trip lifecycle remains unwired into the closed-world pending-trip system. | Slice: replace the pending-trip scheduler with the real `trade.js` `startTrip`/`completeTrip` lifecycle. |
| trade routes | `LIVE_PATH_INTEGRATED` | `routing.js` provides `selectRoute`, `routeCost`, `findRoutePath`, `createRouteBelief`, and is now consumed by the live canonical merchant reroute (base cost + ranking agreement proven). | `findRoutePath` multi-hop planning is unused (all live routes are direct edges). | Slice: use `findRoutePath` when source and destination share no direct road. |
| logistics | `LIVE_PATH_INTEGRATED` | `closed-world.js` owns wagon capacity: `WAGON_CAPACITY = 12` (the established 12-unit shipment is exactly one wagon) with `wagonsForShipment` (`max(1, ceil(amount / 12))`). `schedulePendingTradeTrip` prices road usage per wagon — wear is `0.01 * wagons` (standard loads keep the legacy 0.01 exactly; over-capacity loads degrade faster), floored at 0.5 with the existing maintenance recovery, and `TRIP_COMMITMENT` audits `{wagons, roadCondition}`. Cargo volume is untouched (capacity prices the road, not the goods), so market conservation holds. `tests/logistics-wagon-capacity.test.js` (6 tests) proves wagon math, legacy 12-unit wear with audit, 20-unit double wear, volume preservation, floor behavior, and save/load equality; the Slice Z migration detector now asserts wagon-exact wear per shipped road. | No multi-trip splitting (over-capacity loads travel as one trip with higher wear); no wagon cost in merchant/faction currency. | Slice: split over-capacity loads into convoyed multi-trip shipments, or debit wagon upkeep from a merchant purse. |
...
| refugees | `UNIT_VERIFIED` (immigration only; downgraded 2026-09-03 — no persistent refugee consumer exists) | Immigration is proven (`tests/migration-immigration.test.js`: destinations, conservation); the 3-town case grows a camp only as town population. | No persistent refugee *group*: no `world.refugees` array, no `refugeeGroup` type, no live consumer reads refugee state — immigrants absorb into the destination town. | Slice: add a `world.refugees` array and a `refugeeGroup` type. |
| settlements | `LIVE_PATH_INTEGRATED` | Towns found via `settleAttempt` (knowledge/resource gates), decline and abandon exactly once with claim release, revive husks over sprawl, transfer on WAR-gated takeover contests, revolt back to the recorded prior ruler under sustained brutalization, secede into exactly one newborn sovereign polity per town (deterministic id, booked resource transfer, provisional legitimacy, relationship entry with former-ruler claim, canonical taxation), earn autonomous recognition with pact access from governing well, win external guarantees that lend patron weight to their defense, face former-ruler verdicts (accept / demand-yield / declare with pact-breaking, betrayal withdraws standing), negotiate suzerainty (self-rule under split tribute, overlords restrained, verdicts deferred while honored), federate kin polities for mutual restraint and mutual walls, sell shielded vassalage to strong strangers, lapse barren tribute with renegotiation, stack ordered allegiances, chain overlord-of-overlord hierarchies, support non-territorial nomadic confederations with dynamic seasonal capitals, resolve succession via tribal councils and clan power splits, seal steppe demarcation pacts that protect mutual tributary networks, absorb subordinate hordes into unified Great Khaganates with zero-sum power pooling, contest overlapping seasonal pastures via skirmishes, convene steppe-sedentary peace conferences, and establish frontier trade emporiums with bilateral market trade, raid suppression, and war suspensions (`tests/settlement-staking.test.js`, `tests/settlement-lifecycle.test.js`, `tests/settlement-takeover.test.js`, `tests/revolt-recapture.test.js`, `tests/secession-fragmentation.test.js`, `tests/sovereign-polity-formation.test.js`, `tests/recognition-claims-diplomacy.test.js`, `tests/reconquest-reintegration.test.js`, `tests/patron-guarantee.test.js`, `tests/autonomy-tribute.test.js`, `tests/federation-vassalage.test.js`, `tests/tribute-lapse.test.js`, `tests/layered-allegiances.test.js`, `tests/chained-allegiances.test.js`, `tests/nomadic-confederations.test.js`, `tests/nomadic-succession.test.js`, `tests/nomadic-diplomacy.test.js`, `tests/nomadic-emporiums.test.js`). | Nomadic confederations migrate seasonal capitals between pastures and shelters, extract tribute, mobilize punitive raids, back tributary walls, convene tribal councils, resolve succession disputes with clan power fracturing, seal mutual demarcation pacts protecting tributary networks, absorb subordinate hordes into unified Great Khaganates, contest overlapping seasonal pastures, convene steppe-sedentary peace conferences, and establish frontier trade emporiums with bilateral market trade; seasonal migration valley traversals and mountain pass tolls are a future slice. | Slice: add seasonal migration valley traversals and mountain pass tolls. |
| demography | `CROSS_DOMAIN_INTEGRATED` | The §156 population balance is proven as a property across 100 seeds. | No birth / death / aging model. | Slice: add a `demographicTick(world, rng)` that produces births and deaths based on food availability. |
| ecology | `CONSEQUENCE_VERIFIED` | `ecology.js` owns the season clock (`tickSeason` on the `ticksPerSeason` cadence, `SEASON_CHANGE` on advance) with per-good production modifiers and spoilage modifiers consumed by the step-4 market loop; drought scenario state (`world.drought`) scales town food production with `DROUGHT_STARTED/ENDED` parentage; `demography.js` (`tickDemography`) moves town populations on ecology/scarcity rates with `POPULATION_CHANGE` events. | No wild-resource regeneration model; droughts are scenario-injected, not scheduled. | Slice: add a foraging/regrowth stock for non-town resources, or schedule droughts from season cadence. |
| weather | `LIVE_PATH_INTEGRATED` | Storms are scenario state (`world.storm { active, roadId, severity, remainingTicks, startedTick, startEventId }`), started by direct injection or by the opt-in scheduler (`world.stormSchedule { everyTicks, durationTicks, severity, roadIds, nextRoadIndex? }`, plain JSON). On cadence ticks with no active storm, the reducer starts a storm on the next scheduled road (rotation persists, so save/load cannot double-schedule), emits `STORM_STARTED` with `{scheduled: true}`, and wires `startEventId` for `STORM_ENDED` parentage; active storms are never stacked and malformed schedules are honest no-ops. Absent schedule means no storms — unscheduled worlds behave exactly as before. The reducer prices the storm road at `weatherCost = severity × distance` every tick (other roads reset to 0, older saves migrate with zero weather). Four live consumers: merchants reroute through the Slice AD routing base; `tickBandit` discounts payoff by `distance / (distance + weatherCost)`; `tickPatrol` scales the effective detection rate by the deployed road's factor; and town production scales by `(1 − 0.3 × severity)` on roads incident to the town (milder than drought's 0.6 by design — logistical disruption, not crop failure), flowing into supply through the standard market loop. `tests/storm-weather-routing.test.js` (6), `tests/storm-bandit-suppression.test.js` (6), `tests/storm-patrol-detection.test.js` (6), `tests/storm-scheduler.test.js` (6), and `tests/storm-production.test.js` (6) prove pricing, end/clear with parentage, the merchant flip, the bandit hold, the patrol flip, cadence with rotation and no-stacking, the production ratio (0.82 at severity 0.6) with supply consequence and recovery, incident-only honesty, and save/load equality throughout. | Storms are scheduled, not seasonal/RNG-driven. | Slice: derive schedule timing from season or an RNG stream. |
...
| wildlife | `LIVE_PATH_INTEGRATED` | `wildlife.js` owns `WildlifeGroup` (`{id,roadId,size,lastMoveTick}`, plain JSON) with `createWildlifeGroup`, `tickWildlifeGroup`, `wildlifePressureOnRoad`, `wildlifePayoffFactor`. `createClosedWorldScenario` seeds `wolves-1` (road-b, size 3); the reducer migrates legacy saves (absent means no group, behavior preserved) and ticks groups before the bandit scores routes (deterministic movement toward the busiest merchant road, no RNG consumed, `WILDLIFE_RELOCATION` only on actual moves). `tickBandit` discounts route payoff by `(1 − min(0.8, size/10))` (absent groups factor exactly 1). `tests/wildlife-competition.test.js` (6 tests) proves deterministic tracking, no-traffic hold, crowded-road suppression with empty control, unrelated-road neutrality, exact factor values with cap, and save/load equality with sparse hold. | No reproduction/mortality (size fixed); wildlife does not consume merchant cargo or interact with patrols. | Slice: add predator population dynamics (size from prey density) or patrol-wildlife interaction. |
| infrastructure | `LIVE_PATH_INTEGRATED` | Routes carry plain-number `condition` (1 = pristine, floored at 0.5). `closed-world.js` runs a per-tick maintenance pass (+0.01 toward 1, migrates legacy roads) and applies −0.01 usage wear on every materialized shipment in `schedulePendingTradeTrip`; `TRIP_COMMITMENT` audits the pre-wear `roadCondition`. Condition feeds two live consumers: merchant route scoring (`distanceCost = distance / (10 * condition)`) and travel time (`travelTicks = round(distance / condition)`, worst case 2x), plus `routing.routeCost` (`distance * (1/condition − 1)` surcharge, 0 when absent). `tests/road-condition.test.js` (6 tests) proves pristine init with legacy migration, wear with pre-wear snapshot, recovery with floor, degraded-short-road reroute to pristine-long-road, `routeCost` backward compatibility (5) with floor doubling (10), and 50-tick boundedness with save/load equality. | No multi-segment roads or upgrade investments; wear/recovery rates are heuristic (0.01). | Slice: add road upgrade investment (faction resources → condition) or per-segment conditions. |
...
2. Move that row up at least one evidence level in one bounded
   vertical slice.
3. Re-verify the manual gate (`npm run verify:hard-rule-9 &&
   npm run guardian:check && npm run build`). Do not run `npm test`: it is a
   tombstone that refuses with a non-zero exit (Hard Rule 9).
4. Update this map.
5. Repeat.

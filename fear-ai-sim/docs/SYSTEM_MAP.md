...
- **Unified CLI:** `bin/fear-ai.js` (`npm run cli -- <cmd>`), providing `server`, `dashboard`, `explain`, `explain-faction`, `presets`, `validate-safety`, `frontier-valley`, `counterfactual-world`, `economy`, `diff-replay`, `sim`, `benchmark`, `adversarial`, `counterfactual`, `godot`, `verify`, `governance`, `spatial-3d`, `runaway-loops`, `budget`, `migration`.

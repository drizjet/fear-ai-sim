...
explicit decision (procedure at the bottom). **Re-open executed 2026-09-23:** `Habituation`
left `SOURCE_ABSENT` through that procedure (byte-exact extraction + V8 integration) — the
only row to do so; the manifest below covers the remaining six rows' sources.
...
| Habituation | **EXTRACTED_AND_REOPENED 2026-09-23** — the only re-open executed: source extracted byte-exact to `legacy/habituation.js` (sha256 `df02134b…` verified every gate by `tests/habituation-reopen.test.js`), V8 integration `HabituationBook` + canonical `FEAR_HABITUATED` events; row is `IMPLEMENTED_AND_VERIFIED`. |
...
**Executed once:** `Habituation`, 2026-09-23 — step 1 → `legacy/habituation.js` (sha256

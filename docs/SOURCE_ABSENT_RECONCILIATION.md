...
- `upstream blob` = `git rev-parse origin/master:fear-ai-sim/<file>`; `sha256` = digest of
  `git cat-file blob <blob>`. The document agreement is re-verified on every gate run by
  `npm run gate:check` (`sourceAbsentRows`); the blob+sha256 re-resolution needs the fetched
  `origin/master` and is re-opened by hand (the CI workflow fetches with `fetch-depth: 0`).
...
| Habituation | **EXTRACTED_AND_REOPENED 2026-09-23** — first re-open: source extracted byte-exact to `legacy/habituation.js` (sha256 `df02134b…` verified every gate by the `habituation-reopen` suite (retired 2026-09-23 — `docs/SUITE_RETIREMENT_LOG.md`)), V8 integration `HabituationBook` + canonical `FEAR_HABITUATED` events; row is `IMPLEMENTED_AND_VERIFIED`. |
| Hysteresis | **EXTRACTED_AND_REOPENED 2026-09-23** — second re-open: source extracted byte-exact to `legacy/hysteresis.js` (sha256 `40e5cb26…` verified every gate by the `hysteresis-reopen` suite (retired 2026-09-23 — `docs/SUITE_RETIREMENT_LOG.md`)), V8 integration `HysteresisBook` (asymmetric thresholds, minimum-duration gate, seeded FREEZE roll, world-time records) + canonical `FEAR_STATE_TRANSITION` events; row is `IMPLEMENTED_AND_VERIFIED`. |
...
3. Move the ledger row to `IMPLEMENTED_AND_VERIFIED` **and** update this manifest — `npm run
   gate:check` (`sourceAbsentRows`, whose "a manifest source exists in this checkout" branch is the
   designed tripwire for exactly this act) failing is the signal that the lockstep change was missed.
...
`socialcore.js` consumed by `FEAR_EVENT_RAISED`, pinned by the `habituation-reopen` suite (retired 2026-09-23 — `docs/SUITE_RETIREMENT_LOG.md`);
...
canonical `FEAR_STATE_TRANSITION` event, pinned by the `hysteresis-reopen` suite (retired 2026-09-23 — `docs/SUITE_RETIREMENT_LOG.md`); step 3 →
...
the `fearcore-reopen` suite (retired 2026-09-23 — `docs/SUITE_RETIREMENT_LOG.md`); step 3 → both rows `IMPLEMENTED_AND_VERIFIED`, manifest
...
`COMBAT_ENGAGEMENT` actions, pinned by the `simulation-agents-combat-reopen` suite (retired 2026-09-23 — `docs/SUITE_RETIREMENT_LOG.md`); step 3 →
...
pinned by `npm run gate:check`; step 3 → row `IMPLEMENTED_AND_VERIFIED`, manifest

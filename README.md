...
**Current gate: 154 suites / 475 tests passing** (`npm test`) — as of 2026-09-23.
...
```bash
npm test          # full gate: 154 suites / 475 tests (ESM + Jest via --experimental-vm-modules)
```
...
- **Next responsibility: `RESP-HYSTERESIS-REOPEN-001`** — re-open the `Hysteresis` SOURCE_ABSENT row exactly the way the Habituation re-open proved: extract `hysteresis.js` byte-exact from `origin/master:fear-ai-sim/` (blob `208cbffc038a04a901b262024d5ad414b9dcdcd9`, sha256 already pinned in `docs/SOURCE_ABSENT_RECONCILIATION.md`) with recorded provenance, integrate a V8 hysteresis primitive through canonical events (world-clock only), and move the row `SOURCE_ABSENT → IMPLEMENTED_AND_VERIFIED` through the designed tripwire (manifest + both integrity guards). Closed 2026-09-23: `RESP-FACTION-EVALUATION-RAID-CHAIN-001` (evaluation→raid chain, above), `RESP-FACTION-RAID-LOOP-001` (the orphaned `evaluateRaid`/`raidUtility`/`escalationLevel` macro layer is production-wired — `tests/faction-raid-loop.test.js` 7/7, 7/7 negative controls) and the Habituation re-open (`legacy/habituation.js` byte-exact + `HabituationBook`/`FEAR_HABITUATED` — six `SOURCE_ABSENT` rows remain). Earlier: `RESP-SOURCE-ABSENT-RECONCILIATION-001` (blob+sha256 manifest `docs/SOURCE_ABSENT_RECONCILIATION.md`, guard suite, monorepo option 1 executed — default branch `main`, master untouched) — divergence facts: `docs/MONOREPO_RECONCILIATION.md`.
- Six `SOURCE_ABSENT` rows await their sources actually landing in this checkout (simulation/agents/combat, FearCore, brain, hysteresis, VR/biofeedback, neural fear) — re-open only through the proven procedure (byte-exact extraction + provenance + guard tripwire). CI is `IMPLEMENTED_AND_VERIFIED` (green runs on `main`, cited in the ledger).

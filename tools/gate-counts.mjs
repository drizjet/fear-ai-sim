...
//
// The documents' own invariants — ledger citations, suite references, next responsibility, CI wiring
// — live in `tools/doc-guards.mjs`, so this file stays about one job: the counts the runner produces.
//
...
import { docProblems } from './doc-guards.mjs';
...
export const derive = (root = ROOT) => {
    ...
    // WHICH files failed, not just how many: during a retirement batch the derivation run is red on
    // the gate-agreement suites by construction (they compare the docs against the counts this very
    // run just produced), so the caller has to be able to name what was red before deciding whether
    // it was the batch's own bootstrap artifact or a real failure.
    const failingSuites = report.testResults.filter(result => result.status !== 'passed').map(result => path.basename(result.name)).sort();
    return { suites: report.numTotalTestSuites, tests: report.numTotalTests, failing: report.numFailedTests, failingSuites, casesByFile };
};
...
const main = () => {
    ...
    console.log(`derived from the runner: ${gate.suites} suites / ${gate.tests} tests${derived.failing ? ` (${derived.failing} failing test(s) on this derivation run, in ${derived.failingSuites.length} file(s): ${derived.failingSuites.join(', ')})` : ''}`);

    const problems = [];
    // A count derived from a red run describes a broken gate, so `--check` refuses to bless one. Sync
    // writes regardless — making a stale gate current is what a caller runs it for — and takes its
    // verdict from a SECOND run after the rewrite (below). The derivation run happens while the docs
    // still hold the pre-batch counts, so a retirement legitimately reds the gate-agreement suites;
    // reporting that as a broken gate would make every batch's own evidence ambiguous. Symmetrically,
    // a failure the rewrite does NOT cure is real, and fails this run.
    if (check && derived.failing > 0) problems.push(`the runner reported ${derived.failing} failing test(s) — the gate is red: ${derived.failingSuites.join(', ')}`);
    // A failing CASE count and the list of failing FILES are different numbers; the label must say
    // which is which, or the batch's own evidence is wrong about what happened.
    const failSummary = derivedRun => `${derivedRun.failing} failing test(s) in ${derivedRun.failingSuites.length} file(s): ${derivedRun.failingSuites.join(', ')}`;
    ...
    if (!check) {
        ...
        // The batch's verdict, taken now that the counts are written: the same runner that was red on
        // the stale docs must be green, at the same counts, or the red was not the bootstrap artifact.
        if (derived.failing > 0) {
            const settled = derive(ROOT);
            if (settled.failing > 0) {
                problems.push(`the runner is still red AFTER the rewrite (${failSummary(settled)}) — not the batch's bootstrap artifact`);
            } else if (settled.suites !== derived.suites || settled.tests !== derived.tests) {
                problems.push(`the runner disagrees with itself across two consecutive runs (${derived.suites}/${derived.tests} then ${settled.suites}/${settled.tests}) — re-run before trusting the gate`);
            } else {
                console.log(`the derivation run was red on ${failSummary(derived)} — because the counts it had just produced were not written yet; the rewrite made them current, and the runner is green at ${settled.suites} suites / ${settled.tests} tests`);
            }
        }
    }
    ...
    if (!problems.length) console.log('gate counts current');
        ...
        console.error(check ? 'gate counts are stale — run `npm run gate:sync`' : 'gate counts were rewritten where they could be; the STALE lines above are what the tool could not make current');
        process.exit(1);
    }
};
